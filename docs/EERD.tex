...
\begin{tikzpicture}
    ...
    \node[relation, below=1cm and 1cm of festival] (fest_has) {HAS};
    \node[cardinality, above left=0.3em and -1.6em of fest_has] {1};
    \node[cardinality, below left=0.3em and -1.6em of fest_has] {N};

    \node[entity, below=1cm of fest_has] (category) {CATEGORY};

    \node[attribute, above left=-4mm and 5mm of category] (cid) {\underline{ID}};
    \node[attribute, below=2pt of cid] (cname) {Name};

    \node[relation, below=1cm of category] (belongs_to) {BELONGS\_TO};
    \node[cardinality, above right=-1.5em and 7mm of belongs_to] {N};
    \node[cardinality, above right=1em and -1cm of belongs_to] {1};

    \node[entity, right=2cm of belongs_to] (film) {FILM};

    \node[attribute, below left=-2mm and 12mm of film] (filmid) {\underline{ID}};
    \node[attribute, below=2pt of filmid] (filmtitle) {Title};
    \node[attribute, below=2pt of filmtitle] (filmdur) {Duration};
    \node[attribute, below right=3mm and 10mm of film] (filmyear) {Year};
    ...
    \node[cardinality, below right=1.5mm and -5.5mm of directs] {1};
    ...
    \node[relation, right=1cm of film] (film_has) {HAS};
    ...
    \node[entity, above left=1cm and 1.7cm of staff_spec] (judge) {JUDGE};

    \node[attribute, below left=0.5cm and -1cm of judge] (specialization) {Specialization};

    \node[entity, above right=1cm and 1.5cm of staff_spec] (coordinator) {COORDINATOR};

    \node[entity, right=1cm of film_has] (screening) {SCREENING};
